/*
 * match.cpp -- stolen from match.c in ircd-seven (i.e. charybdis)
 */
...
{
	const char *m = mask.c_str(), *n = name.c_str();
	const char *m_tmp = m, *n_tmp = n;
	int star_p;
 ...
 */
int mask_match(std::string mask, std::string name)
{
	const char *m = mask.c_str(), *n = name.c_str();
	const char *m_tmp = m, *n_tmp = n;
	int star_p;
...
std::string collapse(std::string pattern)
{
	...
	strncpy(buf, pattern.c_str(), pattern.length() + 1);
	char *p = buf, *po = buf;
	char c;
	int f = 0;
